...
if uploaded_file:
    ...
    st.write("🧪 Staging Output:", staging)

    explanation = generate_summary(staging.get('Stage', 'Unknown'), cancer_type)
...
Treatment:
{staging.get('Treatment', 'No treatment recommendation available.')}
...
"""
    ...
    if question == "What is my cancer stage?":
        st.info(f"Stage: {staging.get('Stage', 'Unknown')} (T={staging.get('T', '?')}, N={staging.get('N', '?')}, M={staging.get('M', '?')})")
    elif question == "What treatment is usually given?":
        st.info(staging.get('Treatment', 'No treatment recommendation available.'))
    else:
